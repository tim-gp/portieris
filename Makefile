...
helm.install.local: push
	-rm $$(pwd)/portieris-0.3.0.tgz
	helm package helm/portieris
	helm install -n portieris $$(pwd)/portieris-0.3.0.tgz --set image.host=$(HUB) --set image.tag=$(TAG)

helm.install:
	-rm $$(pwd)/portieris-0.3.0.tgz
	helm package helm/portieris
	helm install -n portieris $$(pwd)/portieris-0.3.0.tgz
...
e2e:
	...
	@go test -v ./test/e2e --helmChart $$(pwd)/portieris-0.3.0.tgz
...
e2e.quick.trust.imagepolicy:
	@go test -v ./test/e2e --no-install --trust-image-policy --helmChart $$(pwd)/portieris-0.3.0.tgz
	-kubectl delete namespace $$(kubectl get namespaces | grep -v ibm | grep -v kube | grep -v default | awk '{ print $$1 }' | grep -v NAME)

e2e.quick.trust.clusterimagepolicy:
	@go test -v ./test/e2e --no-install --trust-cluster-image-policy --helmChart $$(pwd)/portieris-0.3.0.tgz
	-kubectl delete namespace $$(kubectl get namespaces | grep -v ibm | grep -v kube | grep -v default | awk '{ print $$1 }' | grep -v NAME)

e2e.quick.wildcards:
	@go test -v ./test/e2e --no-install --wildcards-image-policy --helmChart $$(pwd)/portieris-0.3.0.tgz
	-kubectl delete namespace $$(kubectl get namespaces | grep -v ibm | grep -v kube | grep -v default | awk '{ print $$1 }' | grep -v NAME)

e2e.quick.armada:
	@go test -v ./test/e2e --no-install --armada --helmChart $$(pwd)/portieris-0.3.0.tgz
	-kubectl delete namespace $$(kubectl get namespaces | grep -v ibm | grep -v kube | grep -v default | awk '{ print $$1 }' | grep -v NAME)

e2e.quick.generic:
	@go test -v ./test/e2e --no-install --generic --helmChart $$(pwd)/portieris-0.3.0.tgz
	-kubectl delete namespace $$(kubectl get namespaces | grep -v ibm | grep -v kube | grep -v default | awk '{ print $$1 }' | grep -v NAME)
